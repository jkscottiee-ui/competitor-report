...
def search_domain_for(url: str) -> str:
    """검색 범위로 쓸 도메인. 오늘의집은 하위 도메인이 여러 개라 상위 도메인까지 넓혀준다."""
    netloc = urlparse(url).netloc
    if netloc.endswith("ohou.se"):
        return "ohou.se"
    return netloc
...
def get_platform_products(company: str, platform: str, url: str) -> list:
    ...
    domain = search_domain_for(url)
    prompt = f"""
가구 브랜드 '{company}'의 '{platform}' 스토어 주소는 아래와 같아:
{url}

이 스토어({domain} 도메인 내)를 웹 검색으로 조사해서, 아래 카테고리에 해당하는 상품을 상위 3~5개 정리해줘:
- 카테고리: {CATEGORY} (다이닝 체어, 스툴, 벤치, 다이닝 세트 등 밀접하게 관련된 상품도 포함해줘. 너무 엄격하게 판단하지 말고, 관련성이 있으면 포함해줘)

브랜드명은 '{company}' 외에 표기가 다를 수 있어 (예: 띄어쓰기, '처'/'쳐' 등 유사 표기). 비슷한 이름으로도 검색해봐.
...
- 검색해도 '{CATEGORY}' 카테고리 상품을 찾을 수 없으면 {{"products": []}}로 응답
"""
    text = ""
    try:
